...
contract GreeterBytes {
    ...
    function greetWithPrefix() public view returns (bytes32) {
        bytes memory prefixed = abi.encodePacked("GR:", greeting);
        return bytes32(bytes31(prefixed));
    }

    function greetWithPrefixBytes() public view returns (bytes memory) {
        bytes memory prefixed = abi.encodePacked("GR:", greeting);
    ...
    function greetWithPrefixStr() public view returns (string memory) {
        // console.log(string(abi.encodePacked(greeting)));
        // uint256 i = 100;
